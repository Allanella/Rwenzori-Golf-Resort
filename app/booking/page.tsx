...
import { Calendar, Users, Bed, Golf, Check, Star, CreditCard, Shield } from 'lucide-react'
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-gradient-to-r from-accent/10 to-primary/10 rounded-lg p-6 border border-accent/20">
                <h3 className="font-serif text-xl font-bold text-foreground mb-4">Booking Summary</h3>
                    ...
                    <>
                      ...
                      {checkIn && checkOut && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            ${rooms.find(r => r.id === selectedRoom)?.price! * Math.ceil((new Date(checkOut).getTime() - new Date(checkIn).getTime()) / (1000 * 60 * 60 * 24))}
                          </span>
                        </div>
                      )}
                      
                      {includeGolf && !rooms.find(r => r.id === selectedRoom)?.features.includes('1 Round Golf') && (
                        <div className="flex justify-between">
                          <span className="text-foreground/80">Golf Rounds</span>
                          <span className="font-medium">
                            ${35 * (checkIn && checkOut ? Math.ceil((new Date(checkOut).getTime() - new Date(checkIn).getTime()) / (1000 * 60 * 60 * 24)) : 0)}
                          </span>
                        </div>
                      )}
                    </>
